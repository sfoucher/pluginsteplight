...
void STL_StepExtractCurvesFromGrid3D::compute()
{
    ...
    for (CT_StandardItemGroup* group : _inGroup.iterateOutputs(_inResult))
    {
        ...
        _tailleConeRecherche = ceil( ( _tailleConeRechercheCm / 100.0 ) / in_Grid3D->resolution() );
        const int nIterMaxGrow = qMax( static_cast<int>(_treeHeightMaximum / in_Grid3D->resolution()), 1);

        // -----------------------------------------------------------------------
        // Get local maximas
        std::vector<Eigen::Vector3i> local_maximas;
        in_Grid3D->get_local_maximas(_grid3DMaximaNeighbourhoodSize,local_maximas,true);
        ...
        RepulseImagePtr repulse_image_ptr = RepulseImage::createGrid3DFromXYZCoords(in_Grid3D->minX(), in_Grid3D->minY(), in_Grid3D->minZ(),
                                                                                      in_Grid3D->maxX(), in_Grid3D->maxY(), in_Grid3D->maxZ(),
                                                                                      in_Grid3D->resolution(),
                                                                                     std::numeric_limits<int>::max(), 0);

        // -----------------------------------------------------------------------
        // Start a growing snake from local maximas
        std::vector<STL_OpenActiveContours<int>*> snakes;
        for( const Eigen::Vector3i& local_maxima : local_maximas )
        {
            ...
            if( repulse_image_ptr->value(local_maxima[0], local_maxima[1], local_maxima[2]) == 0 )
            {
                // Initialise un contours actif a partir du maxima courant
                STL_OpenActiveContours<int>* curr_snake = new STL_OpenActiveContours<int>(in_Grid3D,
                                                repulse_image_ptr,
                                                local_maxima,
                                                20 );
